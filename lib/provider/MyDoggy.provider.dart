...
class MyDoggy {
  ...
  Future<List<User>> getUsersList({int limit = 10, int page = 0}) async {
    var response =
        await _get("/user", params: {"limit": "$limit", "page": "$page"});
  ...
  Future<List<Post>> getPostsList({int limit = 10, int page = 0}) async {
    var response =
        await _get("/post", params: {"limit": "$limit", "page": "$page"});
  ...
  Future<List<Post>> getUserPost(String userId,
    ...
    var response = await _get("/user/$userId/post",
  ...
  Future<List<Comment>> getCommentsList(String postId,
      {int limit = 10, int page = 0}) async {
    var response = await _get("/post/$postId/comment",
        params: {"limit": "$limit", "page": "$page"});
  ...
  Future<List<Post>> getPostbyTag(String tag,
      {int limit = 10, int page = 0}) async {
